...
<script>
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    function closeTip(){const tip=tipBox(),btn=tipBtn();if(!tip||!btn)return;tip.classList.remove('show');btn.setAttribute('aria-expanded','false')}
    function toggleTip(){const tip=tipBox(),btn=tipBtn();if(!tip||!btn)return;const o=tip.classList.toggle('show');btn.setAttribute('aria-expanded',o?'true':'false')}
    ...
    function buildPdfSummary(){
      ...
      if(hasOneTime){
        ...
        if(status==='New'){
          if(useBundle){
            body.appendChild(buildRow('Starter Bundle (5-page site with Google Business Profile)','$'+money(buildP)));
          } else if(buildP>0){
            body.appendChild(buildRow('Website Design','$'+money(buildP)));
          }
        }
